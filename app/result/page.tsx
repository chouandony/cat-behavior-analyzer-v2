  ...
  if (behaviorList.length === 0) {
    return (
      <div className="text-center py-12 space-y-4">
        ...
        <Link href="/abc/" className="inline-block text-cat-600 font-medium hover:underline">
          前往 ABC 分析
        </Link>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-earth-200 p-4 space-y-2">
        <div className="flex flex-wrap gap-2">
          {behaviorList.map(b => (
            <span key={b.id} className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-cat-50 text-cat-700 text-sm font-medium border border-cat-200">
              <span>{b.emoji}</span>
              {b.name}
            </span>
          ))}
