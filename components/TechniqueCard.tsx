...
const categoryColors: Record<string, string> = {
  ...
  '環境設計': 'bg-purple-50 text-purple-600 border-purple-200',
  '進階應用': 'bg-slate-50 text-slate-600 border-slate-200',
  '多貓管理': 'bg-pink-50 text-pink-600 border-pink-200',
  '醫療照護': 'bg-red-50 text-red-600 border-red-200',
}
          ...
          <div className="pt-3 space-y-3">
            <div>
              <h4 className="text-sm font-bold text-earth-500 flex items-center gap-1.5 mb-1">
                <BookOpen size={14} className="text-cat-500" />
                用途
              </h4>
            ...
            <div className="bg-cat-50 rounded-lg p-3 border border-cat-100">
              <h4 className="text-sm font-bold text-cat-700 mb-1">💡 提醒</h4>
              <p className="text-sm text-cat-600 leading-relaxed">{technique.tips}</p>
            </div>
                ...
                <ul className="space-y-1.5">
                  {technique.examples.map((ex, i) => (
                    <li key={i} className="text-sm text-earth-500 flex items-start gap-2">
                      <span className="text-cat-500 shrink-0">•</span>
                      {ex}
                    </li>
                  ))}
                </ul>
